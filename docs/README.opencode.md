# Synapse for OpenCode

Complete guide for using Synapse with [OpenCode.ai](https://opencode.ai).
...
**Skill Priority:** Project skills > Personal skills > Synapse skills

## Updating

Synapse updates automatically when you restart OpenCode. The plugin is re-installed from the git repository on each launch.
...
1. **Injects bootstrap context** via the `experimental.chat.system.transform` hook, adding Synapse awareness to every conversation.
2. **Registers the skills directory** via the `config` hook, so OpenCode discovers all Synapse skills without symlinks or manual config.
...
1. Check OpenCode logs: `opencode run --print-logs "hello" 2>&1 | grep -i synapse`
