...
Add full synapse support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port synapse to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/synapse-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/synapse.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/synapse/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/synapse-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "synapse:brainstorming")')
  }),
...
1. **Inject using-synapse content**
   - Full content of the using-synapse skill
...
```javascript
// .opencode/plugin/synapse.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const SynapsePlugin = async ({ client, directory, $ }) => {
  const synapseDir = path.join(process.env.HOME, '.config/opencode/synapse');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingSynapse = await readSkill('using-synapse');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingSynapse}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── synapse-codex        # UPDATED: Use skills-core
│   ├── synapse-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── synapse.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/synapse-codex`
...
2. Update `.codex/synapse-codex` to use shared core
...
1. Create `.opencode/plugin/synapse.js`
