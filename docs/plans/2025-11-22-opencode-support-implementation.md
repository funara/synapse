...
> **For agentic workers:** REQUIRED SUB-SKILL: Use synapse:executing-plans to implement this plan task-by-task.

**Goal:** Add full synapse support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/synapse-codex` (lines 40-74)
...
- Reference: `.codex/synapse-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'synapse' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/synapse-codex` (lines 212-280)
 ...
 * (personal skills override synapse skills).
 *
 * @param {string} skillName - Name like "synapse:spec" or "my-skill"
 * @param {string} synapseDir - Path to synapse skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, synapseDir, personalDir) {
    // Strip synapse: prefix if present
    const forceSynapse = skillName.startsWith('synapse:');
    const actualSkillName = forceSynapse ? skillName.replace(/^synapse:/, '') : skillName;

    // Try personal skills first (unless explicitly synapse:)
    if (!forceSynapse && personalDir) {
    ...
    // Try synapse skills
    if (synapseDir) {
        const synapsePath = path.join(synapseDir, actualSkillName);
        const synapseSkillFile = path.join(synapsePath, 'SKILL.md');
        if (fs.existsSync(synapseSkillFile)) {
            return {
                skillFile: synapseSkillFile,
                sourceType: 'synapse',
                skillPath: actualSkillName
            };
        }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/synapse-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/synapse-codex` (add import at top)
...
Run: `node -c .codex/synapse-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/synapse-codex
...
**Files:**
- Modify: `.codex/synapse-codex` (lines 40-74)
...
Run: `.codex/synapse-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/synapse-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/synapse-codex` (lines 16-38, approximately)
...
Run: `.codex/synapse-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/synapse.js`
...
/**
 * Synapse plugin for OpenCode.ai
 *
...
const synapseSkillsDir = path.join(homeDir, '.config/opencode/synapse/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const SynapsePlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/synapse.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/synapse.js
...
**Files:**
- Modify: `.opencode/plugin/synapse.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "synapse:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > synapse)
...
Run: `node -c .opencode/plugin/synapse.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the synapse and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const synapseSkills = skillsCore.findSkillsInDir(
            synapseSkillsDir,
            'synapse',
            3
          );
          ...
          const allSkills = [...personalSkills, ...synapseSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install synapse skills to ~/.config/opencode/synapse/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'synapse:';
...
```javascript
    'session.started': async () => {
      // Read using-synapse skill content
      const usingSynapsePath = skillsCore.resolveSkillPath(
        'using-synapse',
        synapseSkillsDir,
        personalSkillsDir
      );

      let usingSynapseContent = '';
      if (usingSynapsePath) {
        const fullContent = fs.readFileSync(usingSynapsePath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingSynapseContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Synapse skills: \`synapse:skill-name\` (from ~/.config/opencode/synapse/skills/)
- Personal skills: \`skill-name\` (from ~/.config/opencode/skills/)
- Personal skills override synapse skills when names match
`;

      // Check for updates (non-blocking)
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/synapse')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/synapse && git pull` to update synapse.' :
        '';
...
**Below is the full content of your 'synapse:using-synapse' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
### 1. Install Synapse Skills

```bash
# Clone synapse skills to OpenCode config directory
mkdir -p ~/.config/opencode/synapse
git clone https://github.com/funara/synapse.git ~/.config/opencode/synapse
```

### 2. Install the Plugin

The plugin is included in the synapse repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/synapse/.opencode/plugin/synapse.js`
...
ln -s ~/.config/opencode/synapse/.opencode/plugin/synapse.js .opencode/plugin/synapse.js
```
...
```
use use_skill tool with skill_name: "synapse:spec"
```
...
Personal skills override synapse skills with the same name.

## Updating

```bash
cd ~/.config/opencode/synapse
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/synapse/.opencode/plugin/synapse.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/synapse/skills`
...
- Documentation: https://github.com/funara/synapse
```
...
Synapse works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/synapse-codex use-skill synapse:spec | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/synapse-codex bootstrap | head -30`
...
ls -l .opencode/plugin/synapse.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/synapse.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/synapse-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/synapse-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/synapse.js` created with tools and hooks
