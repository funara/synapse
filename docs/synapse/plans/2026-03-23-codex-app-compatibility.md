...
> **For agentic workers:** REQUIRED SUB-SKILL: Use synapse:build to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
...
**Spec:** `docs/synapse/specs/2026-03-23-codex-app-compatibility-design.md`
...
| `skills/using-synapse/references/codex-tools.md` | Codex platform reference | Add detection + finishing docs |
...
```
- **synapse:using-git-worktrees** - REQUIRED: Set up isolated workspace before starting
```
To:
```
- **synapse:using-git-worktrees** - REQUIRED: Ensures isolated workspace (creates one or verifies existing)
```
...
**Files:**
- Modify: `skills/using-synapse/references/codex-tools.md:25` (append at end)

- [ ] **Step 1: Read the current file**

Read `skills/using-synapse/references/codex-tools.md` in full. Confirm it ends at line 25-26 after the multi_agent section.
...
```bash
git add skills/using-synapse/references/codex-tools.md
...
- `skills/using-synapse/references/codex-tools.md` — two new sections at end
