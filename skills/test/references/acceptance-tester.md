...
5. Save the report to `docs/synapse/acceptance/reports/` and return it
...
Attempt to invoke `synapse:playwright-cli` before running any `UI interaction` criteria. If the invocation fails or the skill is not found, report the following and mark all `UI interaction` criteria as `Blocked (playwright-cli unavailable)`:
...
Invoke `synapse:playwright-cli` to execute the test.
...
REPORT_PATH="{REPO_ROOT}/docs/synapse/acceptance/reports/${TIMESTAMP}-${FEATURE}.md"
