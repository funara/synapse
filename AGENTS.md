...
This repository is documentation- and skill-first. Core skills live in `skills/<skill-name>/SKILL.md`, with supporting scripts beside them when needed. Command entry docs live in `commands/`. Long-form design and execution records belong under `docs/synapse/specs/` and `docs/synapse/plans/`. Platform-specific integration material lives in `.codex/`, `.claude-plugin/`, `.cursor-plugin/`, and `.opencode/`. Tests are grouped by environment in `tests/claude-code/`, `tests/opencode/`, `tests/skill-triggering/`, `tests/explicit-skill-requests/`, and `tests/brainstorm-server/`.
...
Match the surrounding file style instead of reformatting unrelated content. JavaScript and JSON use 2-space indentation; shell scripts use `set -euo pipefail` and descriptive uppercase variables like `SCRIPT_DIR`. Keep skill folders in kebab-case, for example `skills/test/`. Name docs with date-prefixed kebab-case files such as `docs/synapse/specs/2026-04-04-example-design.md`. No repo-wide formatter or linter is configured, so keep Markdown concise and shell commands copy-pasteable.
...
Repository-level instructions live in `CLAUDE.md`; read them before broad edits. If you add reusable patterns or project conventions, update the appropriate `docs/synapse/reuse/` or standards documentation rather than leaving the knowledge only in code.
